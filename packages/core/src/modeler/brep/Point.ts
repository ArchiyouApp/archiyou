...
import type { MainAxis, PointLike, PolarCoord, 
    Cursor, AnyShapeOrCollection, Plane } from '.' // types

import { AXIS_TO_VECS } from './constants' // see typeguards
import { isPointLike, isCoord, isAxis, isCursor} from './typeguards'

import { Vector, Vertex, Edge, ShapeCollection } from '.'
...
{  
    ...
    fromPointLike( xp?:PointLike, y?:number, z?:number): Point | Vector
    {
        const p = ((y ?? true) || (z ?? true)) ? xp : [xp, y || 0, z || 0]; // if y and z are given, we assume first argument is x and not a PointLike
        ...
        else if(isPointLike(p))
        {
            ...
            else if(isCoord(p)) 
            {
                ...
                let pointArrChecked = this._arrayResolveRelativeCoordinates([p, y || 0, z || 0]);
                [this._x,this._y,this._z] = pointArrChecked;
                return this;
            }
            else if(isAxis(p))
