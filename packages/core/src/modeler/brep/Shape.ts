...
export class Shape
{
    ...
    _updateFromOcShape(ocShape?:any) // TODO: TopoDS_Shape
    {
        ...
        if(ocShape && !ocShape?.IsNull())
        {
            if(USE_GARBAGE_COLLECTION)
            {
                ...
                targetOcForGarbageCollection(ocShape); // set new target
                this.clearMeshCache();
            }
            else {
    ...
    checkAndFix():AnyShape
    {
        ...
        try 
        {
            ...
            if (ocShape && !ocShape.IsNull())
            {
                ...
                targetOcForGarbageCollection(this._ocShape); // set new target
            }
        }
        catch(e)
        {
    ...
    */
    ...
    /** Rotate this Shape by a quaternion around an optional pivot (default: center) */
    rotateQuaternion(
        wOrQuaternion:number|{ w:number, x:number, y:number, z:number },
        xOrPivot?:number|Pivot,
        y?:number,
        z?:number,
        pivot?:Pivot
    ):this
    {
        const quaternion = typeof wOrQuaternion === 'object'
            ? {
                w: wOrQuaternion.w,
                x: wOrQuaternion.x,
                y: wOrQuaternion.y,
            ...
            : {
                w: wOrQuaternion,
                x: xOrPivot as number,
                y: y ?? 0,
                z: z ?? 0,
            };

        if (![quaternion.w, quaternion.x, quaternion.y, quaternion.z].every(value => isNumeric(value)))
        {
            throw new Error('Shape.rotateQuaternion(): expected quaternion components as numbers.');
        }

        const resolvedPivot = typeof wOrQuaternion === 'object'
            ? xOrPivot as Pivot
            : pivot;

        let pivotVec:Vector;

        if (typeof resolvedPivot === 'string' || resolvedPivot == null)
        {
            pivotVec = this.pointAtAlignment(resolvedPivot as string).toVector();
        }
        else
        {
            pivotVec = new Vector(resolvedPivot);
        }

        const ocQuaternion = new this._oc.gp_Quaternion_2(
            quaternion.x,
            quaternion.y,
            quaternion.z,
            quaternion.w
        ...
        ocTransformation.SetRotation_2(ocQuaternion);
    ...
    */
    ...
    equals(other:PointLikeOrAnyShape, tolerance?:number):boolean
    {
        ...
        if (isPointLike(other))
        {
            ...
            return new Point(other).equalsTolerance((this as any as Vertex), tolerance)
        }
